...
pub fn game_phase(board: &Board) -> u32 {
    macro_rules! game_phase_fn {
        ($($piece:ident=$weight:expr,$count:expr;)*) => {
            ...
            let inv_phase = 0 $( + board.pieces(Piece::$piece).len() * $weight)*;
...
impl<'c, T: TraceTarget> EvalContext<'c, T> {
    fn eval(&mut self) -> i16 {
        ...
        let interpolated = ((eval.0 as i32 * (MAX_PHASE as i32 - phase)) + (eval.1 as i32 * phase))
            / MAX_PHASE as i32;
        ...
        for &piece in &Piece::ALL {
            ...
            for square in pieces {
                ...
                eval += *self
                    .weights
                    .piece_tables
                    .get(piece, color, our_king, square);
            }
        }
        eval
    }
        ...
        for &piece in &Piece::ALL {
            ...
            for square in pieces {
                ...
                match piece {
                    ...
                    Piece::Queen => {
                        let piece_attacks =
                            get_rook_moves(square, occupied) | get_bishop_moves(square, occupied);
                        attacks |= piece_attacks;
                        piece_moves |= piece_attacks & !our_pieces;
                    }
                    Piece::King => {
                ...
                let mobility = piece_moves.len() as usize;
                self.trace.trace(|terms| {
    ...
    fn virtual_queen_mobility_terms(&mut self, color: Color) -> PhasedEval {
        ...
        let our_pieces = self.board.colors(color);
        let our_king = self.board.king(color);
        let approx_queen_moves = (get_bishop_moves(our_king, occupied)
            | get_rook_moves(our_king, occupied))
            & !our_pieces;
        let mobility = approx_queen_moves.len() as usize;
        self.trace.trace(|terms| {
    ...
    fn passed_pawn_terms(&mut self, color: Color) -> PhasedEval {
        ...
        for pawn in our_pawns {
            ...
            let passed =
                (their_pawns & blocker_mask).is_empty() && (our_pawns & front_span).is_empty();
            if passed {
                self.trace.trace(|terms| {
    ...
    fn bishop_pair_terms(&mut self, color: Color) -> PhasedEval {
        let mut eval = PhasedEval::ZERO;
        let our_pieces = self.board.colors(color);
        if (our_pieces & self.board.pieces(Piece::Bishop)).len() >= 2 {
            self.trace.trace(|terms| {
    ...
    fn king_ring_attacks_terms(&mut self, color: Color, attacks: BitBoard) -> PhasedEval {
        let our_king = self.board.king(color);
        let attacks = (get_king_moves(our_king) & attacks).len();
        self.trace.trace(|terms| {
